...
GROUP_DEF: dict[str, list[str]] = {
    ...
    "D1": ["ip_reciprocal_friend_count"],
    "D2": ["ip_reciprocal_enemy_count"],
    "D3": ["ip_liked_by_me_but_enemy_to_me_count"],
    "D4": ["ip_enemy_by_me_but_likes_me_count"],
    "D5": ["ip_same_target_friend_and_enemy_count"],
}
...
def setting_label(x: str) -> str:
    mapping = {
        ...
        "drop_plus_A1": "Drop + A1 (OUT)",
        "drop_plus_A2": "Drop + A2 (IN)",
        "drop_plus_B1": "Drop + B1 (OUT)",
        "drop_plus_B2": "Drop + B2 (IN)",
        "drop_plus_C1": "Drop + C1 (OUT)",
        "drop_plus_C2": "Drop + C2 (IN)",
...
def main() -> None:
    ...
    for year, ip_cols in year_to_ip_cols.items():
        ...
        for gname, cols in GROUP_DEF.items():
            gcols[gname] = [c for c in cols if c in ip_cols]
    ...
    for scenario in core.SCENARIOS:
        ...
        settings = [
            ("baseline_drop", []),
            ("drop_plus_A1", gcols["A1"]),
            ("drop_plus_A2", gcols["A2"]),
            ("drop_plus_B1", gcols["B1"]),
            ("drop_plus_B2", gcols["B2"]),
            ("drop_plus_C1", gcols["C1"]),
            ("drop_plus_C2", gcols["C2"]),
    ...
    summary_csv = OUT_ROOT / "a1a2_b1b2_c1c2_d1d5_vs_drop_baseline_summary.csv"
    delta_csv = OUT_ROOT / "a1a2_b1b2_c1c2_d1d5_vs_drop_baseline_deltas.csv"
    detail_json = OUT_ROOT / "a1a2_b1b2_c1c2_d1d5_vs_drop_baseline_details.json"
    report_md = OUT_ROOT / "a1a2_b1b2_c1c2_d1d5_vs_drop_baseline_report_zh.md"
    ...
    report_lines.append("# A1/A2 B1/B2 C1/C2 D1~D5 vs Drop Baseline")
    report_lines.append("")
    report_lines.append("## 1. Group Definition")
    report_lines.append("- A1 (OUT): ip_out_online_friend, ip_out_online_enemy, ip_out_offline_friend, ip_out_offline_enemy")
    report_lines.append("- A2 (IN): ip_in_online_friend, ip_in_online_enemy, ip_in_offline_friend, ip_in_offline_enemy")
    report_lines.append("- B1 (OUT): ip_out_friend_total, ip_out_enemy_total")
    report_lines.append("- B2 (IN): ip_in_friend_total, ip_in_enemy_total")
    report_lines.append("- C1 (OUT): ip_out_friend_online_minus_offline, ip_out_enemy_online_minus_offline")
    report_lines.append("- C2 (IN): ip_in_friend_online_minus_offline, ip_in_enemy_online_minus_offline")
    report_lines.append("- D1~D5: single-feature groups")
    report_lines.append("")

    group_tbl = pd.DataFrame(
        [{"group": "Drop Baseline", "n_features": 0, "features": ""}]
        + [
            {"group": setting_label(f"drop_plus_{g}"), "n_features": len(cols), "features": "; ".join(cols)}
            for g, cols in GROUP_DEF.items()
        ]
    )
    report_lines.append(_md_table(group_tbl, ["group", "n_features", "features"]))
    report_lines.append("")

    report_lines.append("## 2. Overall Mean Performance")
    ...
    overall["setting_label"] = overall["setting"].map(setting_label)
    report_lines.append(_md_table(overall, ["setting_label", "mean_test_accuracy", "mean_test_f1", "mean_test_auc"]))
    report_lines.append("")

    report_lines.append("## 3. Scenario Details")
    for scenario in SCENARIO_ORDER:
        sub = summary_df[summary_df["scenario"] == scenario].copy()
        sub = sub.merge(delta_df[delta_df["scenario"] == scenario], on=["scenario", "setting"], how="left")
        sub = _sort_for_output(sub)
        sub["setting_label"] = sub["setting"].map(setting_label)

        drop_acc = float(sub[sub["setting"] == "baseline_drop"]["test_accuracy"].iloc[0])
        report_lines.append(f"### {scenario_label(scenario)}")
        report_lines.append(f"Drop Baseline test_accuracy: **{drop_acc:.6f}**")
        report_lines.append("")
        report_lines.append(_md_table(
            sub,
            [
                "setting_label",
                "test_accuracy",
    ...
    report_lines.append(f"- summary: `{summary_csv}`")
    report_lines.append(f"- deltas: `{delta_csv}`")
    report_lines.append(f"- details: `{detail_json}`")
    report_lines.append(f"- report: `{report_md}`")
    report_lines.append("")
