...
OUT_DIR = os.path.join(
    BASE_DIR,
    r"Code\Analysis\GNN\Mental分組測試\GCN\Relationshiup_Offline_Like\題組",
)

OUT_METRICS = os.path.join(OUT_DIR, "metrics_gcn.csv")
...
def build_node_table(w2: pd.DataFrame, w3: pd.DataFrame) -> pd.DataFrame:
    ...
    # Scheme A (q<0.05 and valid_row_pct >= 0.5)
...
def main() -> None:
    ...
    # Impute + Standardize features (fit on train only)
