...
OUT_DIR = os.path.join(
    BASE_DIR,
    r"Code\Analysis\GNN\Mental分組測試\GAT\v2\Relationshiup_Offline_Like\單題目",
)
...
OUT_ITEMS = os.path.join(OUT_DIR, "selected_items.csv")
...
def build_node_table(w2: pd.DataFrame, w3: pd.DataFrame) -> pd.DataFrame:
    ...
    # Scheme A filters (q<0.05, valid_row_pct>=0.5)
    ...
    q_w2 = qlist[(qlist["Year"] == "W2") & (qlist["Group_ID"].isin(group_ids))].copy()

    item_cols = q_w2["Question_ID"].dropna().unique().tolist()
    item_cols = [c for c in item_cols if c in w2.columns]

    # Feature matrix: single items
    X = w2[item_cols].apply(pd.to_numeric, errors="coerce")
    X["merge_id"] = w2["student_id"].astype(str).str.strip()
    X["class_id"] = (
        w2["school_id"].astype(str).str.replace(".0", "", regex=False)
        + "_"
        + w2["class"].apply(clean_class)
    )

    data = pd.merge(X, w3_target, on="merge_id", how="inner")
    data = data.dropna(subset=["y_w3_mh", "class_id"])

    # Save item mapping
    q_w2.to_csv(OUT_ITEMS, index=False, encoding="utf-8-sig")
...
def split_by_class(class_ids: np.ndarray, seed: int = 42) -> Dict[str, np.ndarray]:
    classes = pd.unique(class_ids)
    ...
    n_train = int(n * 0.7)
    n_val = int(n * 0.1)
    train_classes = set(classes[:n_train])
    val_classes = set(classes[n_train : n_train + n_val])
    test_classes = set(classes[n_train + n_val :])

    idx = np.arange(len(class_ids))
    train_idx = idx[np.isin(class_ids, list(train_classes))]
    val_idx = idx[np.isin(class_ids, list(val_classes))]
    test_idx = idx[np.isin(class_ids, list(test_classes))]
...
def main() -> None:
    ...
    feature_cols = [c for c in data.columns if c not in {"merge_id", "y_w3_mh", "class_id"}]
    ...
    class_ids = data["class_id"].astype(str).values

    split = split_by_class(class_ids, seed=42)
    ...
    with open(OUT_DATA_SUMMARY, "w", encoding="utf-8") as f:
        ...
        f.write("Class split: yes (school_id + class)\n")
