...
W2_DATA_PATH = (
    BASE_DIR / r"Data\2024data\TIGPS_W2_studentdata_ver7_cleaned_mental_common_only(standerdized).csv"
)
W3_DATA_PATH = (
    BASE_DIR
    / r"Data\2025data\TIGPS_W3_studentdata_ver7_cleaned_cols_removed_missing_common_only(cleaned_q_21)(standerdized).csv"
)
...
def main() -> None:
    ...
    bundle = {
        ...
        "score_definition": "Feature/target scores are row-wise means across their item sets (skipna=True; all-missing rows => NaN).",
    ...
    prefix = "00_ridge_w2_to_w3_target54_selected_w2groups_subscales_ver7"
    ...
    drop_coef_csv = OUT_DIR / f"{prefix}_drop_v57_v52_v51_v50_coefficients.csv"
    full_pred_csv = OUT_DIR / f"{prefix}_full_test_predictions.csv"
    drop_pred_csv = OUT_DIR / f"{prefix}_drop_v57_v52_v51_v50_test_predictions.csv"
    ...
    with open(summary_txt, "w", encoding="utf-8") as f:
        f.write("Ridge Comparison: W2 selected groups (subscale split) -> W3 group 54 (ver7)\n")
        f.write("=" * 74 + "\n")
    ...
    print("Ridge cross-year comparison with subscales completed.")
    ...
    print(
        "Delta (drop-full) test r2/rmse/mae:",
        json.dumps(
            {
                "r2": drop_res["test_metrics"]["r2"] - full_res["test_metrics"]["r2"],
                "rmse": drop_res["test_metrics"]["rmse"] - full_res["test_metrics"]["rmse"],
                "mae": drop_res["test_metrics"]["mae"] - full_res["test_metrics"]["mae"],
            },
            ensure_ascii=False,
        ),
    )
